...
//#region tipo renda
const tipoCustoError = ref("");
const tipoCusto = ref(0);
function verificaTipoRenda() {
  tipoCustoError.value = "";
  if (tipoCusto.value < 1) {
    tipoCustoError.value = "Selecione um tipo de renda";
    return false;
  }
  return true;
}
...
<template>
  <div class="w-full flex flex-col md:flex-row">
    ...
    <div class="form-control w-full md:m-1">
      <label class="label">
        <span class="label-text">Tipo de Custo</span>
      </label>
      <select
        v-model="tipoCusto"
        class="select select-bordered bg-slate-200"
        v-bind:class="{ 'select-error': tipoCustoError != '' }"
      >
        <option :value="0" disabled selected>Escolha</option>
        <option v-for="i of tiposCusto" :key="i.id" :value="i.id">
          {{ i.nome }}
        </option>
      </select>
      <label v-if="tipoCustoError != ''" class="label">
        <span class="label-text-alt text-red-600">{{ tipoCustoError }}</span>
      </label>
    </div>
  </div>
  <div class="w-full flex flex-col md:flex-row">
